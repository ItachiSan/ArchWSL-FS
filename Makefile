...
.target/extract_rootfs: bootstrap_$(ARCH_VERSION).tar.zst
	@echo -e '\e[1;31mExtracting rootfs...\e[m'
	sudo tar -xpf bootstrap_$(ARCH_VERSION).tar.zst root.x86_64
	mkdir -p .target
	touch $@

.target/base_rootfs: .target/extract_rootfs
	@echo -e '\e[1;31mPreparing rootfs folder for work...\e[m'
...
# Rule for making the chroot script available
root.x86_64/usr/bin/arch-chroot: .target/extract_rootfs

scripts/arch-chroot: root.x86_64/usr/bin/arch-chroot
	@echo -e '\e[1;31mFetching arch-chroot script from the tarball...\e[m'
	mkdir -p scripts
	cp root.x86_64/usr/bin/arch-chroot scripts/
...
.target/initial_rootfs: .target/base_rootfs scripts/arch-chroot
...
.target/customized_rootfs: .target/initial_rootfs scripts/arch-chroot .target/installed_fakeroot-tcp glibc-linux4.pkg.tar.zst
